...
canvas.create_text(380,370,text="Player 1",fill='red',font=32)
canvas.create_text(120,370,text="Player 2",fill='red',font=32)
...
class Ball:
    ...
    def score(self,val):
        ...
        global counter1
        if val==True:
            a = self.canvas.create_text(125, 40, text=counter, font=("Arial", 32), fill="white")
            canvas.itemconfig(a,fill="black")
            counter += 1
            a = self.canvas.create_text(125, 40, text=counter, font=("Arial", 32), fill="white")
        if val == False:
            a = self.canvas.create_text(375, 40, text=counter1, font=("Arial", 32), fill="white")
            canvas.itemconfig(a, fill="black")
            counter1 += 1
            a = self.canvas.create_text(375, 40, text=counter1, font=("Arial", 32), fill="white")
...
while 1:
    ...
    if counter==3:
        ...
        paddle.y=0
        paddle1.y=0
        canvas.create_text(250,200,text="Congrats! Player 1 Won",font=32,fill="red")
    ...
    if counter1==3:
        ...
        paddle.y=0
        paddle1.y=0
        canvas.create_text(250,200,text="Congrats! Player 2 Won",font=32,fill="red")
        canvas.create_text(250,215,text="Score " + str(counter) + " - " + str(counter1),font=32,fill="red")
    tk.update_idletasks()
    tk.update()
    time.sleep(0.01)
